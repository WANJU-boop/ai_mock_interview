...
// 计算队列最多存多少个 int16 采样值。录音只需要吸收短暂调度抖动，
// 但火山 TTS 会远快于实时播放速度返回整段问题，因此播放端必须缓存一轮完整语音。
std::size_t queueCapacitySamples(const AudioPcmFormat& format, std::size_t queue_seconds) {
    // queue_seconds 由本文件内固定常量提供；这里仍做乘法溢出检查，避免异常格式触发大分配。
    ...
    if (queue_seconds == 0 ||
        sample_rate > std::numeric_limits<std::size_t>::max() / channels / queue_seconds) {
        return 0;
    }
    ...
    return std::max(frames_per_buffer * channels, sample_rate * channels * queue_seconds);
}

constexpr std::size_t kCaptureQueueSeconds = 2;
// 60 秒单声道 24kHz PCM 约占 2.9 MiB，足够长问题且仍有明确内存上限。
constexpr std::size_t kPlaybackQueueSeconds = 60;
...
bool PortAudioAudioDevice::startCapture(const AudioPcmFormat& format) {
    ...
    // 创建录音队列
    try {
        impl_->capture_queue_ =
            std::make_unique<PcmSampleQueue>(queueCapacitySamples(format, kCaptureQueueSeconds));
        //
...
bool PortAudioAudioDevice::startPlayback(const AudioPcmFormat& format) {
    ...
    // 创建播放队列
    try {
        impl_->playback_queue_ =
            std::make_unique<PcmSampleQueue>(queueCapacitySamples(format, kPlaybackQueueSeconds));
    } catch (const std::exception&) {
...
bool PortAudioAudioDevice::hasPendingPlayback() const {
    return impl_->playback_started_ && impl_->playback_queue_ != nullptr &&
           impl_->playback_queue_->availableSamples() > 0;
}
