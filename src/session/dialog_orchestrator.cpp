...
DialogOrchestratorResult DialogOrchestrator::run() {
    ...
    if (!realtime_client_.connect()) {
        ...
        failSession(result, realtime_client_, audio_bridge_, "realtime 会话连接失败。");
        return result;
    }
    ...
    // 当前 orchestrator 仍是同步事件循环：mock 测试能确定性复现，真实 WebSocket adapter
    // 可以阻塞读取。audio_bridge 也只能由这个同一线程调用，保证 WSS 的 read/write/close
    ...
    while (realtime_client_.hasNextEvent()) {
        ...
        case common::RealtimeEventType::kConnected:
            ...
            transitionState(result.session, InterviewState::kInterviewerSpeaking);
            if (!sendInterviewerText(result, realtime_client_, audio_bridge_,
