...
- 创建合并请求（Merge Request, MR）并准备合并到 `main` 时，默认将所有项目导师添加为 reviewer；导师名单以当前项目平台成员和项目约定为准。
- 创建 MR 后，Codex 只需提醒用户把 MR 链接发送到项目群，并 @ 项目导师；群内通知和消息发送由用户自己负责。
- MR 提交完成后，如果需要开始下一轮开发，先执行 `git checkout main` 回到主分支，再执行 `git pull` 同步远端最新代码。
